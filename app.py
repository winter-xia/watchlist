...
# 在扩展类实例化前加载配置
db = SQLAlchemy(app)
login_manager = LoginManager(app)  # 实例化扩展累
login_manager.login_view = 'login'  # 如果未登录的用户访问对应的 URL，Flask-Login 会把用户重定向到登录页面
...
# 返回渲染好的模板作为响应
@app.route('/sofia', methods=['GET', 'POST'])
def index():
	if request.method == 'POST':
